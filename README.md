...
1. Select the disk you would like to make available within ESXi as a VMFS datastore
1. Set a root password or use the example password we will use throughout this series `changethisp455word!`. If you use a custom password, be sure to update the config files in later steps
1. Cofirm the install by hitting `F11` when prompted
1. Remove the install media and hit `Enter` to reboot when prompted
...
1. Install vCenter
    ...
    ```
1. After 10 or 20 minutes, the installer should complete
1. Launch [https://vcsa-01.lab](https://vcsa-01.lab) and test a login with `administrator@vsphere.local` and `changethisP455word!`
